...
```{r}
write.csv(clinic, "/Users/matthewgenchev/Desktop/USC/qbio_490_matthewgenchev/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```
...
colnames(clinic) and rownames(clinic)
```{r Exercise 3.2}
colnames(clinic)
rownames(clinic)
```
...
clinic$vital_status or clinic[ , 7]
```{r Exercise 3.3}
clinic$vital_status
```
...
plot( x = clinic$days_to_birth,
      y = clinic$days_to_last_followup,
      main = "days to last followup vs days to birth", 
      xlab = "days to birth", 
...
one categorical and one discrete
For an extra challenge, add a plot title, axes titles, and confidence intervals to your plot.

```{r Exercise 4.2}
boxplot( clinic$age_at_initial_pathologic_diagnosis ~ clinic$menopause_status,
         main = "Age at diagnosis vs Menopause Status",
...
```{r}
under30_over60_mask <- ifelse(clinic$age_at_initial_pathologic_diagnosis < 30 | clinic$age_at_initial_pathologic_diagnosis > 60, TRUE, FALSE)
```
